...
export class CameraPicsService {
  ...
  /**
   * Get presigned URL for a thumbnail image using the thumbnail endpoint
   * @param developerTag - Developer tag (not ID)
   * @param projectTag - Project tag (not ID)
   * @param cameraTag - Camera tag (camera name, not ID)
   * @param imageTimestamp - Image timestamp (YYYYMMDDHHMMSS format, e.g., "20251230021102")
   */
  getThumbnailPresignedUrl(developerTag: string, projectTag: string, cameraTag: string, imageTimestamp: string): Observable<string> {
    const url = `${getApiUrl('/api/camerapics-s3-test')}/thumbnail/${developerTag}/${projectTag}/${cameraTag}/${imageTimestamp}`;
    
    return this.http.get<{ url: string; key: string; expiresIn: number }>(url).pipe(
      map(response => response.url),
      catchError(error => {
        console.error('Error fetching thumbnail presigned URL:', error);
        return of('');
      })
    );
  }
