...
export class ProjectDetailComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  // Cache for thumbnail presigned URLs
   ...
   */
  private fetchPresignedUrlsForBatch(
    timestamps: string[],
    developerTag: string,
    projectTag: string,
    cameraTag: string,
    ...
    timestamps.forEach((timestamp, index) => {
      ...
      if (this.imageUrlCache.has(cacheKey)) {
        ...
        if (loadedCount === totalCount && onComplete) {
          ...
          console.log(`✅ Batch fetch complete: ${loadedCount}/${totalCount} URLs in ${duration.toFixed(2)}ms`);
          onComplete();
        }
        return;
      }
      ...
      // This returns: https://s3.ap-southeast-1.idrivee2.com/camera-pictures/upload/.../large/timestamp.jpg?[auth]
      ...
      setTimeout(() => {
        this.http.get<{ url: string; key: string; expiresIn: number }>(url, { headers: this.getAuthHeaders() }).subscribe({
          next: (response) => {
            ...
            img.onload = () => {
              ...
              if (loadedCount === totalCount) {
                const duration = performance.now() - startTime;
                console.log(`✅ Batch fetch complete: ${loadedCount}/${totalCount} presigned URLs fetched and images preloaded in ${duration.toFixed(2)}ms`);
                if (onComplete) onComplete();
              }
            };
            img.onerror = () => {
              ...
              if (loadedCount === totalCount) {
                const duration = performance.now() - startTime;
                console.log(`⚠️ Batch fetch complete with errors: ${loadedCount}/${totalCount} URLs in ${duration.toFixed(2)}ms`);
                if (onComplete) onComplete();
              }
            };
          ...
          error: (err) => {
            loadedCount++;
            console.warn(`Failed to fetch presigned URL for ${timestamp}:`, err);
            if (loadedCount === totalCount) {
              const duration = performance.now() - startTime;
              console.log(`⚠️ Batch fetch complete with errors: ${loadedCount}/${totalCount} URLs in ${duration.toFixed(2)}ms`);
              if (onComplete) onComplete();
      ...
      }, index * 50); // 50ms stagger to avoid overwhelming
    });
  }

  /**
   * Fetch presigned URL for a single image
   */
  private fetchPresignedUrl(
    ...
    if (this.imageUrlCache.has(cacheKey)) {
      return Promise.resolve(this.imageUrlCache.get(cacheKey) || null);
    }
  ...
  /**
   * Get image URL with automatic fetching - uses presigned URLs from /image endpoint
   */
  private getImageUrlWithFetch(
    timestamp: string,
    developerTag: string,
    projectTag: string,
    cameraTag: string
    ...
    // Return cached presigned URL immediately if available
    if (this.imageUrlCache.has(cacheKey)) {
      return this.imageUrlCache.get(cacheKey) || '';
    }
    
    // If not cached, fetch presigned URL asynchronously
    // For now, return empty string - the image will be updated once the URL is fetched
    this.fetchPresignedUrl(timestamp, developerTag, projectTag, cameraTag).then(url => {
      if (url) {
        this.cdr.markForCheck();
      }
    }).catch(() => {
      // If fetching presigned URL fails, fall back to proxied URL
    });
  ...
  /**
   * Check if we have a cached presigned URL for a timestamp
   */
  private hasPresignedUrl(
    timestamp: string,
    developerTag: string,
    projectTag: string,
    cameraTag: string
   ...
   */
  private fetchThumbnailPresignedUrlsForBatch(
    timestamps: string[],
    developerTag: string,
    projectTag: string,
    cameraTag: string,
  ...
  /**
   * Get thumbnail URL with automatic fetching - uses presigned URLs from /thumbnail endpoint
   */
  private getThumbnailUrlWithFetch(
    timestamp: string,
    developerTag: string,
    projectTag: string,
    cameraTag: string
    ...
    // Return cached thumbnail presigned URL immediately if available
    if (this.thumbnailUrlCache.has(cacheKey)) {
      return this.thumbnailUrlCache.get(cacheKey) || '';
    }
    
    // If not cached, fetch thumbnail presigned URL asynchronously
    // For now, return empty string - the thumbnail will be updated once the URL is fetched
    this.fetchThumbnailUrl(timestamp, developerTag, projectTag, cameraTag);
    
    return '';
  }

  /**
   * Fetch thumbnail presigned URL for a single image
   */
  private fetchThumbnailUrl(
    timestamp: string,
    developerTag: string,
    projectTag: string,
    cameraTag: string
    ...
    // Return cached URL if available
    if (this.thumbnailUrlCache.has(cacheKey)) {
      return Promise.resolve(this.thumbnailUrlCache.get(cacheKey) || null);
    }
  ...
  loadProject() {
    ...
    forkJoin({
      ...
      next: ({ project, serviceConfig }) => {
          ...
          forkJoin({
            ...
            next: (results) => {
              ...
              if (this.cameras.length > 0) {
                ...
                // Thumbnails are loaded directly from S3 in loadImagesWithTags
              }
            },
            error: (err) => {
        ...
        // Set banner image directly
        this.bannerImage = project.image || null;
  ...
  loadCameras() {
    ...
    // Don't set isLoadingCameras to true - show cameras immediately with thumbnails
    this.error = null;
  ...
  loadImagesWithTags(cameras: Camera[], developerTag: string, projectTag: string) {
    ...
    const imageRequests = cameras.map(camera => {
      ...
      if (cachedTimestamp) {
        // Return cached timestamp - we'll fetch thumbnail directly from S3
        return of({
          cameraId: camera.id,
      ...
      return this.cameraPicsService.getCameraPictures(
        ...
        catchError(error => {
          ...
          return of({ cameraId: camera.id, lastPhotoTimestamp: '', path: '' });
        })
      );
    });

    // Don't mark cameras as loading - show them immediately with thumbnails
    // Subscribe to each request immediately so thumbnails load as soon as available
    // This provides progressive loading - thumbnails appear immediately without blocking UI
        ...
        next: (result) => {
          ...
          if (camera) {
            ...
            if (result.lastPhotoTimestamp) {
              ...
              // Fetch thumbnail directly from S3
              if (result.lastPhotoTimestamp) {
                this.fetchThumbnailUrl(result.lastPhotoTimestamp, developerTag, projectTag, camera.camera).then(thumbnailUrl => {
                  if (thumbnailUrl) {
                    camera.thumbnail = thumbnailUrl;
                    this.cdr.markForCheck();
                  }
                }).catch(() => {
                  // Thumbnail fetch failed - set no image
  ...
  onFullImageLoad(event: Event, cameraId: string) {
    // Full image loaded - fade in to replace thumbnail
    const img = event.target as HTMLImageElement;
    if (img) {
      img.style.opacity = '1';
    }
   ...
   */
  /**
   * Preload image using thumbnails first (faster initial loading), then full image
   * Falls back to proxied URL if presigned URL fails
   */
  private preloadImageWithPresignedUrl(
    camera: Camera,
    timestamp: string,
    developerTag: string,
    projectTag: string,
    ...
    if (!timestamp || timestamp.length < 14) {
      // Invalid timestamp, fall back to proxied URL
    ...
    // Step 1: Load thumbnail first for instant visual feedback (much faster!)
    const thumbnailCacheKey = cacheKey;
    if (this.thumbnailUrlCache.has(thumbnailCacheKey)) {
      const thumbnailUrl = this.thumbnailUrlCache.get(thumbnailCacheKey)!;
      // Set thumbnail immediately for fast loading
      camera.thumbnail = thumbnailUrl;
      this.cdr.markForCheck();
    } else {
      // Fetch thumbnail presigned URL first
      this.fetchThumbnailUrl(timestamp, developerTag, projectTag, cameraTag).then(thumbnailUrl => {
        if (thumbnailUrl) {
          camera.thumbnail = thumbnailUrl;
          this.cdr.markForCheck();
        }
      }).catch(() => {
        // Thumbnail fetch failed, continue with full image loading
      });
    }
    
    // Step 2: Load full image in the background
    if (this.imageUrlCache.has(cacheKey)) {
      const presignedUrl = this.imageUrlCache.get(cacheKey)!;
      const img = new Image();
      
      img.onload = () => {
        // Full image loaded successfully - replace thumbnail with full image
        camera.image = presignedUrl;
        if (!camera.thumbnail) {
          camera.thumbnail = presignedUrl; // Fallback: use full image as thumbnail if thumbnail failed
        }
        console.log(`✓ Full image loaded successfully for camera ${camera.name}`);
        this.cdr.markForCheck();
      };
      
      img.onerror = () => {
        // Presigned URL failed, fall back to proxied URL
        console.warn(`Presigned S3 URL failed for camera ${camera.name}, falling back to proxied URL`);
        this.preloadImageWithFallback(camera, timestamp, developerTag, projectTag, cameraTag);
      };
    ...
    // Fetch full image presigned URL, then load
    this.fetchPresignedUrl(timestamp, developerTag, projectTag, cameraTag).then(presignedUrl => {
      if (presignedUrl) {
        const img = new Image();
        ...
        // Failed to get presigned URL, fall back to proxied URL
        this.preloadImageWithFallback(camera, timestamp, developerTag, projectTag, cameraTag);
      }
    }).catch(() => {
      // Failed to fetch presigned URL, fall back to proxied URL
      this.preloadImageWithFallback(camera, timestamp, developerTag, projectTag, cameraTag);
    });
  }
  ...
  /**
   * Check if all camera images are loaded (for internal tracking only, no UI blocking)
   */
  private checkAllImagesLoaded() {
    // No longer blocking UI - images load progressively with thumbnails first
  }
    ...
    const preloadRequests = activeCameras.map(camera => {
      ...
      return this.cameraPicsService.getCameraPictures(developerTag, projectTag, camera.camera!, todayStr, todayStr).pipe(
        map(response => {
          ...
          if (allPhotos.length > 0) {
            // Step 1: Batch fetch thumbnails first (faster initial loading!)
            ...
            // Step 2: Batch fetch full image presigned URLs from /image endpoint
            this.fetchPresignedUrlsForBatch(
              allPhotos,
              developerTag,
              projectTag,
              camera.camera!,
              () => {
                // After presigned URLs are fetched, cache them with timestamps
                const imageUrls = allPhotos.map(timestamp => {
                  const cacheKey = `${developerTag}/${projectTag}/${camera.camera!}/${timestamp}`;
                  // Return presigned URL if cached, otherwise fallback to proxied URL
                  return this.imageUrlCache.get(cacheKey) || 
                    this.cameraPicsService.getProxiedImageUrl(developerTag, projectTag, camera.camera!, timestamp);
                });
            ...
            // Use proxied URLs as initial placeholders (will be replaced with presigned URLs once fetched)
            const placeholderUrls = allPhotos.map(timestamp => 
              this.cameraPicsService.getProxiedImageUrl(developerTag, projectTag, camera.camera!, timestamp)
            );

            // Cache placeholder URLs initially
            this.cacheService.setDateImages(
              developerTag,
              projectTag,
              camera.camera!,
            ...
            console.log(`Preloading ${allPhotos.length} thumbnails and full images for ${camera.name} (${todayStr}) using presigned S3 URLs`);
          } else {
                ...
                next: (yesterdayResponse) => {
                  ...
                  if (yesterdayPhotos.length > 0) {
                    ...
                    this.fetchPresignedUrlsForBatch(
                      ...
                      camera.camera!,
                      () => {
                        // After presigned URLs are fetched, cache them
                    ...
                    // Use proxied URLs as placeholders initially
                    const placeholderUrls = yesterdayPhotos.map(timestamp => 
                      this.cameraPicsService.getProxiedImageUrl(developerTag, projectTag, camera.camera!, timestamp)
                    );
                    this.cacheService.setDateImages(
                      developerTag,
                    ...
                    console.log(`Preloaded ${yesterdayPhotos.length} thumbnails and full images for ${camera.name} (${yesterdayStr}) using presigned S3 URLs`);
                  }
                }
              });
            }
