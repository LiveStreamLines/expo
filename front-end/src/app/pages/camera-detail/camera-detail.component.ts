import { Component, OnInit, OnDestroy, AfterViewChecked, ViewChild, ElementRef, HostListener, ChangeDetectorRef } from '@angular/core';
...
export class CameraDetailComponent implements OnInit, OnDestroy, AfterViewChecked {
  ...
  videoSpeed: string = 'regular'; // 'fast', 'regular', or 'slow'
  imageSize: string = 'large'; // 'optimized' or 'large'
  ...
  videoBrightness: number = 50; // Default to 50 (neutral brightness = 0.0)
  videoContrast: number = 50;   // Default to 50 (neutral contrast = 1.0)
  videoSaturation: number = 50; // Default to 50 (neutral saturation = 1.0)
  estimatedVideoDuration: number | null = null; // Estimated duration in seconds
  ...
  showVideoResolutionDropdown: boolean = false;
  showVideoSpeedDropdown: boolean = false;
  showVideoImageSizeDropdown: boolean = false;
  ...
  // Presigned URL caching (similar to camera-pics-s3-test)
  private imageUrlCache: Map<string, string> = new Map();
  // Thumbnail URL cache for faster initial loading
  ...
  // Separate array for thumbnail URLs in the strip
  ...
  loadCameraData() {
    ...
    this.camerasService.getCameraById(this.cameraId!).subscribe({
      next: (camera) => {
        ...
        // Load images with last photo date/time initialization
        this.loadImagesForSelectedDate(camera, true);
      },
      error: (err) => {
  ...
  /**
   * Initialize selectedDateObj from timestamp (YYYYMMDDHHMMSS format)
   */
  private initializeDateFromTimestamp(timestamp: string | null) {
    if (timestamp && timestamp.length >= 14) {
      ...
      this.selectedDateObj = new Date(year, month, day, hour, minute, second);
  ...
  loadImagesForSelectedDate(camera: Camera, isInitialLoad: boolean = false) {
    ...
    forkJoin({
      ...
      next: ({ project, developer }) => {
        ...
        if (developerTag && projectTag && cameraTag) {
          // On initial load, get last photo timestamp and set date/time
          if (isInitialLoad) {
            this.getLastPhotoAndSetDate(developerTag, projectTag, cameraTag, camera);
          } else {
            // Use existing selectedDateObj
  ...
  /**
   * Get last photo timestamp and set initial date/time, then load images
   */
  private getLastPhotoAndSetDate(developerTag: string, projectTag: string, cameraTag: string, camera: Camera) {
    // Try to get last photo timestamp
    this.cameraPicsService.getLastImageUrl(developerTag, projectTag, cameraTag).subscribe({
      next: (imageUrl) => {
        ...
        // If not found in URL, try to get from camera pics service
        if (!lastTimestamp) {
          this.cameraPicsService.getCameraPictures(developerTag, projectTag, cameraTag).subscribe({
            next: (response) => {
              if (response.lastPhoto) {
                lastTimestamp = response.lastPhoto;
              }
              this.initializeDateFromTimestamp(lastTimestamp);
              this.loadImagesForDateAfterInit(developerTag, projectTag, cameraTag);
            },
            error: () => {
              // Fallback to today if can't get last photo
              this.selectedDateObj = new Date();
              this.updateDateDisplay();
              this.loadImagesForDateAfterInit(developerTag, projectTag, cameraTag);
            }
      ...
      error: () => {
        // Fallback to today if can't get last image
        this.selectedDateObj = new Date();
        this.updateDateDisplay();
        this.loadImagesForDateAfterInit(developerTag, projectTag, cameraTag);
      }
  ...
  /**
   * Load images after date initialization
   */
  private loadImagesForDateAfterInit(developerTag: string, projectTag: string, cameraTag: string) {
    const selectedDateStr = this.formatDateToYYYYMMDD(this.selectedDateObj);
    const todayStr = this.formatDateToYYYYMMDD(new Date());
    ...
    this.http.post<any>(url, { date1: todayStr, date2: todayStr }).subscribe({
      next: (response) => {
        ...
        if (allPhotos.length > 0) {
          ...
          // Initialize images array with empty strings - will be populated with presigned URLs as they're fetched
          this.images = new Array(sortedPhotos.length).fill('');
          // Initialize thumbnail images array for faster initial loading
          this.thumbnailImages = new Array(sortedPhotos.length).fill('');
          
          // Use presigned URLs from cache if available, otherwise wait for batch fetch
          sortedPhotos.forEach((timestamp, index) => {
            const cachedUrl = this.getImageUrlWithFetch(timestamp, developerTag, projectTag, cameraTag);
            if (cachedUrl) {
              this.images[index] = cachedUrl;
            }
            // Don't use proxied URLs as fallback - wait for presigned URLs to be fetched
          });
          
          this.currentImageIndex = sortedPhotos.length - 1; // Start with latest
          
          // Batch fetch thumbnails first (faster initial loading!), then full images
          this.fetchPresignedUrlsForBatch(sortedPhotos, developerTag, projectTag, cameraTag, () => {
            // All images loaded - set progress to 100% and hide loading immediately
            this.loadingProgress = 100;
            this.isLoading = false;
  ...
  loadImagesForDate(developerTag: string, projectTag: string, cameraTag: string, dateStr: string) {
    ...
    if (cached) {
      ...
      this.loadingProgress = 100;
      this.images = cached.imageUrls;
      // Initialize thumbnails from cache if available, otherwise use images as fallback
      this.thumbnailImages = cached.imageUrls; // Use full images as thumbnails if thumbnails not available
      this.isLoading = false;
      return;
    }
  ...
  onDateChange(event: Event) {
    ...
    if (input.value) {
      ...
      // Hide thumbnail strip when date changes
  ...
  openVideoGenerationModal() {
    ...
    this.showVideoResolutionDropdown = false;
    this.showVideoSpeedDropdown = false;
    this.showVideoImageSizeDropdown = false;
    ...
    this.videoSpeed = 'regular';
    this.imageSize = 'large';
    this.estimatedVideoDuration = null;
  ...
  /**
   * Toggle video dropdown menus
   */
  toggleVideoDropdown(type: 'resolution' | 'speed' | 'imageSize', event: Event): void {
    event.stopPropagation();
    ...
    switch (type) {
      case 'resolution':
        this.showVideoResolutionDropdown = !this.showVideoResolutionDropdown;
        break;
      case 'speed':
        this.showVideoSpeedDropdown = !this.showVideoSpeedDropdown;
        break;
      case 'imageSize':
        this.showVideoImageSizeDropdown = !this.showVideoImageSizeDropdown;
        break;
    }
  }
  ...
  generateVideo() {
    ...
    // Format dates as YYYY-MM-DD for new S3 endpoint
    ...
    // For brightness: 0-100 maps to -1.0 to 1.0 (50 = 0.0, neutral)
    // For contrast/saturation: 0-100 maps to 0.0 to 2.0 (50 = 1.0, neutral)
    const convertBrightness = (value: number): string => {
      // Convert from 0-100 scale to -1.0 to 1.0 scale (50 = 0.0)
      const normalized = ((value - 50) / 50).toFixed(1);
      return normalized;
    };
    
    const convertContrastSaturation = (value: number): string => {
      // Convert from 0-100 scale to 0.0 to 2.0 scale (50 = 1.0, neutral)
      const normalized = (value / 50).toFixed(1);
      return normalized;
    };

    const startDate = formatDateToYYYYMMDD(this.videoFromDate);
    const endDate = formatDateToYYYYMMDD(this.videoToDate);
    const startTime = String(this.videoFromHour).padStart(2, '0');
    const endTime = String(this.videoToHour).padStart(2, '0');
    const showdate = this.showDateInVideo || this.showTimeInVideo; // Show date if either is enabled
    const topText = this.videoOverlayType === 'text' ? this.videoTextOverlay : '';
    const resolution = this.videoResolution;
    const speed = this.videoSpeed;
    const imageSize = this.imageSize;
    const contrast = convertContrastSaturation(this.videoContrast);
    const brightness = convertBrightness(this.videoBrightness);
    const saturation = convertContrastSaturation(this.videoSaturation);
    ...
    if (this.videoOverlayType === 'watermark' && this.videoWatermarkFile) {
      formData.append('watermark', this.videoWatermarkFile);
    }
    ...
    // Make API call to new S3 endpoint
    const url = `${API_CONFIG.baseUrl}/api/video/generateFromS3`;
    this.http.post<any>(url, formData, { headers }).subscribe({
      next: (response) => {
        this.isGeneratingVideo = false;
        if (response.success) {
          const durationMinutes = Math.floor(response.estimatedDuration / 60);
          const durationSeconds = response.estimatedDuration % 60;
          const durationText = durationMinutes > 0 
            ? `${durationMinutes} minute${durationMinutes > 1 ? 's' : ''} ${durationSeconds > 0 ? durationSeconds + ' second' + (durationSeconds > 1 ? 's' : '') : ''}`
            : `${durationSeconds} second${durationSeconds > 1 ? 's' : ''}`;
          
          this.videoGenerationSuccess = `Video generated successfully! ${response.imageCount || 0} images processed. Estimated duration: ${durationText}. File size: ${response.fileSize || 'N/A'}`;
          ...
          this.videoGenerationError = response.message || 'Video generation failed.';
        }
      },
      error: (error) => {
        this.isGeneratingVideo = false;
  ...
  /**
   * Batch fetch thumbnail presigned URLs for multiple images (faster initial loading)
   * Uses the /thumbnail endpoint which returns presigned S3 URLs for thumbnails
   */
  fetchThumbnailPresignedUrlsForBatch(
    timestamps: string[],
    developerTag: string,
    projectTag: string,
    cameraTag: string,
    ...
    console.log(`🚀 Batch fetching ${totalCount} thumbnail presigned URLs...`);

    // Initialize thumbnailImages array with empty strings
    this.thumbnailImages = new Array(totalCount).fill('');

    // Fetch thumbnail presigned URLs for all images in parallel
    timestamps.forEach((timestamp, index) => {
      const cacheKey = `${developerTag}/${projectTag}/${cameraTag}/${timestamp}`;
      
      // Skip if already cached
      if (this.thumbnailUrlCache.has(cacheKey)) {
        const thumbnailUrl = this.thumbnailUrlCache.get(cacheKey)!;
        this.thumbnailImages[index] = thumbnailUrl;
        loadedCount++;
        if (loadedCount === totalCount && onComplete) {
          const duration = performance.now() - startTime;
          console.log(`✅ Thumbnail batch fetch complete: ${loadedCount}/${totalCount} URLs in ${duration.toFixed(2)}ms`);
          this.cdr.markForCheck();
          onComplete();
        }
        return;
      ...
      // Use the /thumbnail endpoint to get presigned URL
      const url = `${getApiUrl('/api/camerapics-s3-test')}/thumbnail/${developerTag}/${projectTag}/${cameraTag}/${timestamp}`;

      // Make requests in parallel with minimal stagger to avoid overwhelming the server
      setTimeout(() => {
        this.http.get<{ url: string; key: string; expiresIn: number }>(url, { headers: this.getAuthHeaders() }).subscribe({
          next: (response) => {
            // Cache the thumbnail presigned URL
            this.thumbnailUrlCache.set(cacheKey, response.url);
            this.thumbnailImages[index] = response.url;
            
            // Pre-load the thumbnail into browser cache for instant display
            const img = new Image();
            img.onload = () => {
              loadedCount++;
              ...
              if (loadedCount === totalCount) {
                const duration = performance.now() - startTime;
                console.log(`✅ Thumbnail batch fetch complete: ${loadedCount}/${totalCount} thumbnails fetched and preloaded in ${duration.toFixed(2)}ms`);
                this.cdr.markForCheck();
                if (onComplete) onComplete();
              }
            };
            img.onerror = () => {
              loadedCount++;
              console.warn(`Failed to preload thumbnail for ${timestamp}`);
              if (loadedCount === totalCount) {
                const duration = performance.now() - startTime;
                console.log(`⚠️ Thumbnail batch fetch complete with errors: ${loadedCount}/${totalCount} URLs in ${duration.toFixed(2)}ms`);
                this.cdr.markForCheck();
                if (onComplete) onComplete();
              }
            };
          ...
          error: (err) => {
            loadedCount++;
            console.warn(`Failed to fetch thumbnail presigned URL for ${timestamp}:`, err);
            if (loadedCount === totalCount) {
              const duration = performance.now() - startTime;
              console.log(`⚠️ Thumbnail batch fetch complete with errors: ${loadedCount}/${totalCount} URLs in ${duration.toFixed(2)}ms`);
              this.cdr.markForCheck();
      ...
      }, index * 10); // 10ms stagger to avoid overwhelming (reduced from 50ms for faster loading)
    });
  }

  /**
   * Batch fetch presigned URLs for multiple images (much faster than one-by-one)
   * Uses the /image endpoint which returns presigned S3 URLs with authentication
   * Tracks image loading progress and calls onAllImagesLoaded callback when done
   * Also fetches thumbnails first for faster initial loading
   */
  fetchPresignedUrlsForBatch(
    timestamps: string[],
    developerTag: string,
    projectTag: string,
    cameraTag: string,
    ...
    // Step 1: Fetch thumbnails first for instant visual feedback (much faster!)
    this.fetchThumbnailPresignedUrlsForBatch(
      timestamps,
      developerTag,
      projectTag,
      cameraTag,
      () => {
        // Thumbnails loaded, now load full images
        this.fetchFullImagePresignedUrlsForBatch(
          timestamps,
          developerTag,
          projectTag,
          cameraTag,
  ...
  /**
   * Batch fetch full image presigned URLs for multiple images
   * Internal method - called after thumbnails are loaded
   */
  private fetchFullImagePresignedUrlsForBatch(
    timestamps: string[],
    developerTag: string,
    projectTag: string,
    cameraTag: string,
    ...
    this.totalImageCount = totalCount;
    this.loadingProgress = 0;
    
    console.log(`🚀 Batch fetching ${totalCount} presigned URLs using /image endpoint...`);

    // Fetch presigned URLs for all images in parallel (with slight staggering to avoid overwhelming)
    timestamps.forEach((timestamp, index) => {
      const cacheKey = `${developerTag}/${projectTag}/${cameraTag}/${timestamp}`;
      const cachedUrl = this.imageUrlCache.get(cacheKey);
      
      // If already cached and preloaded, count it immediately
      if (cachedUrl && this.preloadedImages.has(timestamp)) {
        urlLoadedCount++;
        imageLoadedCount++;
        this.loadedImageCount++;
        this.updateLoadingProgress();
        
        // Update images array with cached presigned URL
        const timestampIndex = timestamps.indexOf(timestamp);
        if (timestampIndex !== -1) {
          this.images[timestampIndex] = cachedUrl;
          this.cdr.markForCheck();
        }
        
        if (urlLoadedCount === totalCount && imageLoadedCount === totalCount) {
          const duration = performance.now() - startTime;
          console.log(`✅ Batch fetch complete: ${imageLoadedCount}/${totalCount} URLs fetched and images preloaded in ${duration.toFixed(2)}ms`);
          if (onAllImagesLoaded) {
            onAllImagesLoaded();
          }
        }
      ...
      // If cached but not preloaded, use cached URL and preload it
      if (cachedUrl && !this.preloadedImages.has(timestamp)) {
        urlLoadedCount++;
        ...
        if (timestampIndex !== -1) {
          ...
          // Hide loading once first URL is available
          if (urlLoadedCount === 1 && this.isLoading) {
            this.isLoading = false;
            this.loadingProgress = Math.min(95, Math.round((urlLoadedCount / totalCount) * 100));
          }
          
          this.cdr.markForCheck();
        }
        
        // Pre-load the image into browser cache (background task)
        const img = new Image();
        img.onload = () => {
          imageLoadedCount++;
          this.loadedImageCount++;
          ...
          if (imageLoadedCount === totalCount) {
            const duration = performance.now() - startTime;
            console.log(`✅ Batch fetch complete: ${imageLoadedCount}/${totalCount} presigned URLs fetched and images preloaded in ${duration.toFixed(2)}ms`);
            this.isLoading = false;
            this.loadingProgress = 100;
            if (onAllImagesLoaded) {
              onAllImagesLoaded();
        ...
        img.onerror = () => {
          ...
          console.warn(`Failed to preload cached image for ${timestamp}`);
      ...
      // Use the /image endpoint to get presigned URL
      const url = `${getApiUrl('/api/camerapics-s3-test')}/image/${developerTag}/${projectTag}/${cameraTag}/${timestamp}`;
      ...
      setTimeout(() => {
        this.http.get<{ url: string; key: string; expiresIn: number }>(url, { headers: this.getAuthHeaders() }).subscribe({
          next: (response) => {
            ...
            // Update images array with the new presigned URL (replace empty string or proxied URL)
            const timestampIndex = timestamps.indexOf(timestamp);
            if (timestampIndex !== -1) {
              this.images[timestampIndex] = response.url;
              this.cdr.markForCheck();
              
              // Hide loading once first few URLs are available (don't wait for all)
              if (urlLoadedCount === 1 && this.isLoading) {
                // First URL loaded - hide loading immediately for better UX
                this.isLoading = false;
                this.loadingProgress = Math.min(95, Math.round((urlLoadedCount / totalCount) * 100));
                this.cdr.markForCheck();
              }
            ...
            // Pre-load the image into browser cache for instant display (background task)
            const img = new Image();
            img.onload = () => {
              imageLoadedCount++;
              this.loadedImageCount++;
              ...
              this.preloadedImages.set(timestamp, img);
                ...
                if (imageLoadedCount === totalCount) {
                  const duration = performance.now() - startTime;
                  console.log(`✅ Batch fetch complete: ${imageLoadedCount}/${totalCount} presigned URLs fetched and images preloaded in ${duration.toFixed(2)}ms`);
                  console.log(`   Average: ${(duration / imageLoadedCount).toFixed(2)}ms per image`);
                  
                  // Ensure loading is hidden and progress is 100%
                  this.isLoading = false;
                  this.loadingProgress = 100;
            ...
            img.onerror = () => {
              ...
              console.warn(`Failed to preload image for ${timestamp}`);
              
              if (imageLoadedCount === totalCount) {
                const duration = performance.now() - startTime;
                console.log(`⚠️ Batch fetch complete with errors: ${imageLoadedCount}/${totalCount} URLs in ${duration.toFixed(2)}ms`);
                
                // Call callback even if some images failed
                if (onAllImagesLoaded) {
                  onAllImagesLoaded();
                }
              }
          ...
          error: (err) => {
            ...
            console.error(`Error fetching presigned URL for ${timestamp}:`, err);
            this.imageUrlCache.set(cacheKey, '');
            
            // Fallback to proxied URL if presigned URL fetch fails
            const timestampIndex = timestamps.indexOf(timestamp);
            if (timestampIndex !== -1 && this.images[timestampIndex] === '') {
              this.images[timestampIndex] = this.cameraPicsService.getProxiedImageUrl(developerTag, projectTag, cameraTag, timestamp);
              this.cdr.markForCheck();
            }
  ...
  private updateLoadingProgress(): void {
    if (this.totalImageCount > 0) {
      this.loadingProgress = Math.min(100, Math.round((this.loadedImageCount / this.totalImageCount) * 100));
      this.cdr.markForCheck();
    }
  }

  /**
   * Fetch presigned URL for an image and pre-load it (single image)
   */
  fetchImageUrl(timestamp: string, developerTag: string, projectTag: string, cameraTag: string): void {
    const cacheKey = `${developerTag}/${projectTag}/${cameraTag}/${timestamp}`;
   ...
   * Uses presigned URLs from /image endpoint (faster than proxy, proper S3 authentication)
   */
  getImageUrlWithFetch(timestamp: string, developerTag: string, projectTag: string, cameraTag: string): string {
    const cacheKey = `${developerTag}/${projectTag}/${cameraTag}/${timestamp}`;
    ...
    // If not cached, fetch presigned URL asynchronously
    this.fetchImageUrl(timestamp, developerTag, projectTag, cameraTag);
    
    // Return empty string for now - will update once presigned URL is fetched
    return '';
  }

  /**
   * Check if image URL is available (presigned URL from /image endpoint)
   */
  hasImageUrl(timestamp: string, developerTag: string, projectTag: string, cameraTag: string): boolean {
    const cacheKey = `${developerTag}/${projectTag}/${cameraTag}/${timestamp}`;
    return this.imageUrlCache.has(cacheKey) && !!this.imageUrlCache.get(cacheKey);
  }

  /**
   * Handle thumbnail load in strip
   */
  onThumbnailLoad(event: Event, index: number) {
  ...
  /**
   * Handle full image load in strip - fade in to replace thumbnail
   */
  onFullImageLoadInStrip(event: Event, index: number) {
    const img = event.target as HTMLImageElement;
    if (img) {
      img.style.opacity = '1';
      // Full image is now visible, fade out thumbnail (optional - can keep both for smoother transition)
    }
  }

  /**
   * Get image URL for a timestamp (for thumbnail display)
   * Now uses presigned URLs from /image endpoint instead of proxied URLs
   */
  getCompareImageUrl(timestamp: string): string {
    ...
    // Use presigned URL from cache or fetch it
    return this.getImageUrlWithFetch(timestamp, developerTag, projectTag, cameraTag) || 
  ...
  toggleCompareDatePicker(side: 'left' | 'right', event: Event) {
    ...
    if (side === 'left') {
      ...
      if (this.showCompareLeftDatePicker) {
        ...
        // Also open the thumbnail strip when date picker opens
        this.showCompareLeftThumbnailStrip = true;
      }
    } else {
      this.showCompareLeftDatePicker = false;
